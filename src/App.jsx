...
const ROWS = [
  ...
  { id: "hollywood", label: "🎬 Hollywood Blockbusters", url: API + "/movie/popular?api_key=" + KEY + "&with_original_language=en" },
  { id: "anime", label: "⛩️ Anime", url: API + "/discover/tv?api_key=" + KEY + "&with_genres=16&with_origin_country=JP&sort_by=popularity.desc" },
  { id: "korean", label: "🇰🇷 Korean Hits", url: API + "/discover/movie?api_key=" + KEY + "&with_original_language=ko&sort_by=popularity.desc" },
  { id: "tamil", label: "🎭 Tamil Movies", url: API + "/discover/movie?api_key=" + KEY + "&with_original_language=ta&sort_by=popularity.desc" },
  { id: "top_rated", label: "⭐ Top Rated", url: API + "/movie/top_rated?api_key=" + KEY },
  { id: "tv", label: "📺 Popular TV Shows", url: API + "/tv/popular?api_key=" + KEY },
];
          ...
          {hovered && (
            <div style={{ marginTop: 6, animation: "fadeUp 0.2s ease" }}>
              ...
              <button onClick={(e) => { e.stopPropagation(); onInfo(item, kind); }}
                style={{ marginTop: 6, width: "100%", padding: "5px", background: "#e50914", border: "none", color: "#fff", borderRadius: 5, fontSize: 10, fontWeight: 700, cursor: "pointer" }}>▶ Play</button>
            </div>
          )}
        </div>
      </div>
      <button onClick={e => { e.stopPropagation(); onToggle(item, kind); }}
        style={{ position: "absolute", top: 6, right: 6, background: saved ? "#e50914" : "rgba(0,0,0,0.7)", border: "none", color: "#fff", width: 26, height: 26, borderRadius: "50%", cursor: "pointer", fontSize: 12, transition: "all 0.2s", transform: saved ? "scale(1.1)" : "scale(1)" }}>
        {saved ? "♥" : "♡"}
      </button>
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 32, animation: "fadeUp 0.5s ease" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px", marginBottom: 10 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: "#fff", letterSpacing: "-0.3px" }}>{section.label}</div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={() => scroll(-1)} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid #333", color: "#fff", width: 28, height: 28, borderRadius: "50%", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
          <button onClick={() => scroll(1)} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid #333", color: "#fff", width: 28, height: 28, borderRadius: "50%", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
        </div>
      </div>
...
function Player({ item, kind, season, episode, onClose }) {
  const url = (kind === "tv" || kind === "anime_tv")
    ? "https://www.2embed.cc/embedtv/" + item.id + "&s=" + (season || 1) + "&e=" + (episode || 1)
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000", zIndex: 9999, display: "flex", flexDirection: "column", animation: "fadeIn 0.3s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", background: "linear-gradient(90deg, #111, #1a1a1a)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => { if (document.documentElement.requestFullscreen) document.documentElement.requestFullscreen(); }}
            style={{ background: "#222", border: "1px solid #333", color: "#fff", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>⛶ Full</button>
      ...
      <div style={{ flex: 1, position: "relative" }}>
        <iframe src={url} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", border: "none" }} allowFullScreen allow="autoplay; fullscreen; picture-in-picture" title={item.title || item.name} />
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 14, animation: "fadeIn 0.25s ease" }}>
        <div onClick={e => e.stopPropagation()} style={{ background: "linear-gradient(180deg, #0d0d18, #080810)", borderRadius: 20, overflow: "hidden", maxWidth: 680, width: "100%", maxHeight: "90vh", overflowY: "auto", position: "relative", border: "1px solid #1a1a2e", boxShadow: "0 30px 80px rgba(0,0,0,0.8)" }}>
          ...
          <button onClick={onClose} style={{ position: "absolute", top: 12, right: 12, background: "rgba(0,0,0,0.7)", border: "1px solid #333", color: "#fff", width: 34, height: 34, borderRadius: "50%", cursor: "pointer", fontSize: 17, zIndex: 10, transition: "all 0.2s" }}>×</button>
          <div style={{ padding: "14px 16px 10px", display: "flex", gap: 14, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 150 }}>
              <div style={{ fontSize: 18, fontWeight: 900, color: "#fff", marginBottom: 6, lineHeight: 1.2 }}>{title}</div>
              <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <span style={{ fontSize: 10, color: "#e50914", background: "rgba(229,9,20,0.15)", padding: "2px 8px", borderRadius: 12, border: "1px solid rgba(229,9,20,0.3)" }}>HD</span>
              </div>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                {!isTv && (
                  <button onClick={() => { onClose(); onPlay(item, kind, null, null); }}
                    style={{ padding: "9px 18px", background: "linear-gradient(135deg, #e50914, #ff6b6b)", border: "none", color: "#fff", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 16px rgba(229,9,20,0.4)", transition: "all 0.2s" }}>
                    ▶ Watch Now
                  </button>
                )}
                {trailerKey && (
                  <button onClick={() => setShowTrailer(true)}
                    style={{ padding: "9px 14px", background: "rgba(255,255,255,0.08)", border: "1px solid #333", color: "#fff", borderRadius: 8, fontSize: 12, cursor: "pointer", transition: "all 0.2s" }}>
                    🎬 Trailer
                  </button>
                )}
                <button onClick={() => onToggle(item, kind)}
                  style={{ padding: "9px 14px", background: saved ? "rgba(229,9,20,0.2)" : "rgba(255,255,255,0.05)", border: "1px solid " + (saved ? "#e50914" : "#333"), color: saved ? "#e50914" : "#ccc", borderRadius: 8, fontSize: 12, cursor: "pointer", transition: "all 0.2s" }}>
                  {saved ? "♥ Saved" : "♡ Save"}
                </button>
              </div>
            </div>
          ...
          {isTv && seasons.length > 0 && (
            <div style={{ padding: "8px 16px 16px" }}>
              ...
              {loadingEp ? <div style={{ color: "#555", fontSize: 12 }}>Loading...</div> : (
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {episodes.map(ep => (
                    <div key={ep.id} onClick={() => { onClose(); onPlay(item, kind, selSeason, ep.episode_number); }}
                      ...
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg, #e50914, #ff6b6b)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#fff", flexShrink: 0, boxShadow: "0 4px 12px rgba(229,9,20,0.3)" }}>{ep.episode_number}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#555", marginTop: 1 }}>{ep.air_date || ""}</div>
                      </div>
...
function HeroSection({ onInfo }) {
  ...
  useEffect(() => {
    fetch(API + "/trending/movie/week?api_key=" + KEY)
      .then(r => r.json())
      .then(d => setHeroes(d.results?.slice(0, 5) || []))
      .catch(() => {});
  }, []);
  ...
  return (
    <div style={{ position: "relative", height: 480, overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: 50, left: 0, padding: "0 20px", maxWidth: 500 }}>
        <div style={{ fontSize: 9, color: "#e50914", letterSpacing: 4, fontWeight: 800, marginBottom: 8, animation: "fadeUp 0.5s ease" }}>🔥 TRENDING NOW</div>
        <div style={{ fontSize: 34, fontWeight: 900, color: "#fff", lineHeight: 1.1, marginBottom: 10, animation: "fadeUp 0.5s ease 0.1s both" }}>{hero.title || hero.name}</div>
        <div style={{ animation: "fadeUp 0.5s ease 0.2s both" }}>
          <Stars n={hero.vote_average} />
        </div>
        <p style={{ fontSize: 12, color: "#bbb", lineHeight: 1.65, margin: "8px 0 14px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", animation: "fadeUp 0.5s ease 0.3s both" }}>{hero.overview}</p>
        <div style={{ display: "flex", gap: 10, animation: "fadeUp 0.5s ease 0.4s both" }}>
          ...
          <button onClick={() => onInfo(hero, "movie")}
            style={{ padding: "10px 18px", background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.25)", color: "#fff", borderRadius: 8, fontSize: 13, cursor: "pointer", backdropFilter: "blur(4px)", transition: "all 0.2s" }}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080810", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      {/* NAVBAR */}
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "linear-gradient(to bottom, rgba(8,8,16,0.98), rgba(8,8,16,0.7))", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.04)", padding: "10px 16px" }}>
...
function SectionPage({ tab, onInfo, onToggle, inWl }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(buildUrl(page, sec, genre))
      .then(r => r.json())
      .then(d => { setItems(d.results || []); setPages(Math.min(d.total_pages || 1, 20)); setLoading(false); })
      .catch(() => setLoading(false));
  }, [tab, page, sec, genre]);
        ...
        {!loading && items.length > 0 && (
          <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 24, alignItems: "center" }}>
            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1}
              style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #222", background: "transparent", color: page===1?"#333":"#ccc", cursor: page===1?"default":"pointer", fontSize: 12, transition: "all 0.2s" }}>← Prev</button>
            <span style={{ color: "#444", fontSize: 12 }}>{page} / {pages}</span>
            <button onClick={() => setPage(p => Math.min(pages, p+1))} disabled={page===pages}
              style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #222", background: "transparent", color: page===pages?"#333":"#ccc", cursor: page===pages?"default":"pointer", fontSize: 12, transition: "all 0.2s" }}>Next →</button>
          </div>
        )}
      </div>
    </div>
